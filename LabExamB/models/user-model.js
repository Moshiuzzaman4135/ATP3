...
module.exports ={
	...
	insertEmp: function(employees, callback){
		var sql = "insert into employees values(?,?,?,?)";
		db.execute(sql, [null,employees.name,employees.companyname,employees.contactno,employees.username,employees.password], function(status){
			if(status){
				callback(true);
			}else{
				callback(false);
